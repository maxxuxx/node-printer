...
import type { CupsPrinterDependencies } from "#cups";
import type { NetworkPrinterDependencies } from "#network";
import type { SerialPrinterDependencies } from "#serial";
import type { WinspoolBinding } from "#winspool";
...
// 비-Windows에서 winspool native가, CUPS 명령 경로가 미리 로드되지 않도록 격리함
...
export function createPrinter(
  ...
  switch (targetType) {
    ...
    case "cups":
      return new LazyPrinter(target, async () => {
        const { createCupsPrinter } = await import("#cups");
