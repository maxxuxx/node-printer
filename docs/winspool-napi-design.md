...
- Visual Studio `cl`과 `link`로 prebuilt binary를 생성합니다
...
모듈은 Windows 전용 private transport로 설계합니다
...
모듈의 TypeScript API는 transport에 맞게 단순하게 유지합니다
...
- Visual Studio `cl`과 `link`로 Windows ia32, x64, arm64 artifact 생성
...
N-API 호환 범위는 prebuild 스크립트의 `NAPI_VERSION=3` 정의로 시작합니다
...
prebuild 래퍼는 `vswhere`로 Visual Studio C++ 구성요소가 설치된 경로를 찾고 `vcvarsall.bat` 이후 `cl`과 `link`를 직접 호출합니다

ARM64 교차 빌드는 x64 호스트 도구와 ARM64 도구를 함께 요구합니다

산출물 구조는 런타임 로더가 찾는 prebuild 디렉터리를 사용합니다
...
기본 파일명은 `@node-printer+winspool.node`이고, 런타임 로더는 해당 디렉터리의 `.node` 파일도 안정적인 순서로 탐색해 로드합니다

npm 패키지에는 `dist/**`와 `prebuilds/**/*.node`만 포함하고 `native/`, `scripts/`, `build/`, `.pdb`, `.iobj`, `.ipdb`는 포함하지 않습니다

설치 스크립트와 소스 빌드 폴백은 두지 않습니다
